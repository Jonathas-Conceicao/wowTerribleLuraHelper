...
## Current State: v1.1.0 QoL Update — Shipped 2026-05-16

Latest milestone shipped. Two quality-of-life improvements on top of the v1.0.0 foundation: the helper window now auto-opens/closes as you enter and leave the March of Quel'danas raid (any difficulty — single instanceID 2913 covers LFR/Story/Normal/Heroic/Mythic), and a new "Use verbose markers" toggle lets English-client groups opt into `{diamond}` / `{triangle}` / `{circle}` / `{cross}` macro payloads. Default OFF — `{rt#}` codes are universal across all WoW client locales.

Next milestone TBD. See `MILESTONES.md` for full v0.1.0 / v1.0.0 / v1.1.0 history and `RETROSPECTIVE.md` for cross-milestone lessons.
...
**v1.0.0 milestone complete (2026-05-10) — 15/15 requirements shipped across Phases 4, 5, 6.**

- ✓ **Verbose-marker toggle (Phase 7)** — New `db.verboseMarkers` boolean (default OFF) controls whether the four marker macros emit verbose tokens (`{diamond}` / `{triangle}` / `{circle}` / `{cross}`) or the universal `{rt#}` codes. New "Use verbose markers" checkbox in the config panel (Macros section, between channel dropdown and Recreate button) with a locale-aware tooltip leading with the English-client-only warning. Toggle change rebuilds the four marker macros in place via the existing `RegisterMacros` path with combat-lockdown deferral; prints `TLH: Verbose markers on/off. Macros updated.` (with deferred-in-combat variant). Dual-field `MACROS` table (payloadVerbose + payloadRT per marker row); TLH_T keeps its single literal-"T" payload. SAFE-06-compliant nil-check backfill so existing users land on the universal default. Default flipped from initial `true` → `false` during UAT (AMEND-07-01-UAT-01) after the localization implication was confirmed as the dominant factor for raid groups with mixed-locale players. Reqs satisfied: MACR-06, MACR-07, CFG-15, CFG-16, SCAF-18. **Phase 7 complete 2026-05-16.**
- ✓ **Zone-aware auto show/hide (Phase 8)** — Permanent `zoneFrame` listener on `PLAYER_ENTERING_WORLD` + `ZONE_CHANGED_NEW_AREA` (Blizzard's `MapTexturePreloader.lua` dual-event pattern). Handler `ns:OnZoneChanged()` queries `GetInstanceInfo()` and dispatches to existing `ns:ShowWindow()` / `ns:HideWindow()` based on `instanceType == "raid"` AND `instanceID == 2913` (the M of Q raid instanceID — captured live during UAT via gated `DEBUG_ZONE_INFO` flag; two-track placeholder pattern with sentinel `0` ensured zero regression risk pre-capture). `IsInInstance()` fast-path early-returns to `ns:HideWindow()` on outdoor zone-border crossings, avoiding `GetInstanceInfo()` on the hot path. Difficulty-agnostic — single scalar covers all five Midnight raid difficulties (LFR / Story / Normal / Heroic / Mythic). AMEND-01 invariant preserved (zone-leave calls full `Hide()`, never `SetAlpha(0)`); SCAF-19 localization-safety enforced (no `GetZoneText` / `GetRealZoneText` / `GetMinimapZoneText` in the auto-handler). Reqs satisfied: WIN-16, WIN-17, SCAF-19, SAFE-07. **Phase 8 complete 2026-05-16.**

**v1.1.0 milestone complete (2026-05-16) — 9/9 requirements shipped across Phases 7, 8.**

### Active

v1.1.0 milestone complete (2026-05-16). No active requirements; next milestone TBD.
...
| Semver policy: minor bumps (`v1.X.0`) for any new features (incl. QoL); patch bumps (`v1.0.X`) reserved for hotfixes only | v1.1.0 contains user-facing features (verbose-marker toggle, zone-aware auto show/hide) — that's a minor, not a patch. Originally tagged as v1.0.1, renamed 2026-05-15 before execution. | ✓ Adopted 2026-05-15 |
...
| v1.1.0: Phase 8 uses `GetInstanceInfo()` instanceID, NOT `C_Map.GetBestMapForUnit` uiMapID | uiMapID can split per raid sub-area (historical Sunwell precedent: 335 + 336); instanceID is shared across all raid difficulties. Pivot keeps `LURA_RAID_INSTANCE_ID` a single scalar covering all 5 Midnight difficulties (LFR/Story/Normal/Heroic/Mythic) instead of a per-difficulty lookup table. | ✓ Shipped Phase 8 |
| v1.1.0: Phase 8 two-track placeholder pattern for unknown-at-plan-time constants | The actual M of Q raid instanceID wasn't pinned from documented sources at plan time. Shipped with sentinel `LURA_RAID_INSTANCE_ID = 0` (safe no-op — never matches a real raid) + gated `DEBUG_ZONE_INFO` debug-print block. UAT captured `inst=2913` from a live LFR zone-in; follow-up `fix(08-01)` patched the constant + flipped the debug flag off. Zero regression risk during the placeholder window. | ✓ Shipped Phase 8 (live capture 2026-05-16) |
| v1.1.0: verbose-marker toggle default is OFF (universal `{rt#}`), opt-in for English-only groups | Verbose tokens (`{diamond}` etc.) only render on English WoW clients (locale-loaded `ICON_TAG_LIST` global strings); `{rt#}` is universal. Default flipped from initial `true` → `false` during UAT (AMEND-07-01-UAT-01) after the localization implication played out in practice. Tooltip rewritten to lead with the English-client warning. | ✓ Shipped Phase 7 (AMEND-07-01-UAT-01) |
| v1.1.0: UAT batched across phases (Phase 7 deferred to combine with Phase 8) | Pattern established in v1.0.0 (Phase 5+6 combined UAT per RETROSPECTIVE). For v1.1.0, Phase 7 shipped code-complete-UAT-deferred; UAT ran as a single 16-checkpoint in-game session at end of milestone (Block A: Phase 7; Block B: instanceID capture; Block C: Phase 8). Surfaced the verbose-default reframe AMEND-07-01-UAT-01 mid-session. Saves context-clearing cycles between phases. | ✓ Validated across v1.0.0 + v1.1.0 |
...
---
*Last updated: 2026-05-16 — v1.1.0 milestone complete. Phases 7 + 8 shipped; 9/9 requirements validated. Squash-merged `milestone/1.1.0` → `main` and tagged `v1.1.0`.*
