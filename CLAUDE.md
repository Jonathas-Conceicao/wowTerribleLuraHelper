...
- **Chat events should be registered only while the addon is in the *enabled* state AND combat is active.** The on/off gating mirrors the POC's NSRT-style model — register on `PLAYER_REGEN_DISABLED`, unregister on `PLAYER_REGEN_ENABLED`, and skip both if the user has run `/lura hide`.
